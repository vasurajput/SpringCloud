...
public class UserDao {
	...
	public int insertUser() {
		try {
			int updateStatus = template.update("insert into user(username,password,enabled) values(?,?,?)", new Object[] {"vishu",encoder.encode("rajput"),"1"});
			logger.info("vvv::  updateStatus= "+updateStatus);
			if(updateStatus == 1) {
				int insertIntoAuthTable = template.update("insert into authorities(username,authority) values(?,?)", new Object[] {"vishu","ROLE_USER"});
